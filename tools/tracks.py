    ...
    try:
        ...
        with torch.no_grad():
            ...
            for batch in tqdm(data_loader):
                ...
                for frame in frames: # Push values between 0-256 and convert masks to bytes
                    frame['masks'] *= 255.99 
                ...
                for res in list_res:
                    ...
                    for key in res:
                        ...
                        if key == 'labels':
                            ...
                            cls_map_cuda = cls_map.to(res[key].device)  # Move cls_map to CUDA
                            res[key] = cls_map_cuda[res[key].to(res[key].device)]  # Now both tensors are on the same device
...
def track_cars( video ):
    try:
        ...
        tracks = tracker.track_vehicles(detections, kf_params=kf_params, max_age=5, min_hits=1, update_box=True)
